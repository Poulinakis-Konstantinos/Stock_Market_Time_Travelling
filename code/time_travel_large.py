...
def intraday( single_date, portfolio, account, today_state, margine_called):
    ...
    for poss in possesions_copy.keys(): 
        ...
        if (not poss_today.empty):
            ...
            # if I did not intra-trade buy return false
            return False
    return False
...
def margin_call(today_state, portfolio, account, date):
    ...
    for poss in possesions_copy.keys(): 
        ...
        if (type(success)==bool) : portfolio.log_transaction(single_date, 'sell-open', poss, N) ; print("MAAAAAAAAAAARRRRRRRRGGGGGGGGGGGGIIIIIIINNNNNNNNN CAAAAAAAAAAAAAAAAAAAAAAL")
...
probs = [0.75, 0.125, 0.125] # probabilities for buy,sell
# intraday increase 
global incr 
incr = 1 - 0.005
...
for i, single_date in enumerate(dates.unique()): 
    ...
    # Start mass selling
    if ((year == 2016) and (month == 1) and (day == 25)):
         liquidate = True
    if (liquidate):
...
ax.step(x=plot_dates, y=acc_net_worth,  where='post', color='blue', label='Balance', alpha=1.0)
ax.step(x=plot_dates, y=port_net_worth, where='post', color='orange', label='Portfolio')
ax.set_yscale('log')
...
plt.suptitle('Portfolio and Account Net worth through time (Log Scaled)', fontsize=16)
# Add legend to plot 
legend = ax.legend() 
